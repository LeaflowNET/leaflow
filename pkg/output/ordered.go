...
func sortKeys(object map[string]any) []string {
	...
	sort.Slice(keys, func(i, j int) bool {
		return sortsBefore(keys[i], keys[j])
	})
...
func encodeJSONObject(buffer *bytes.Buffer, object map[string]any, indent, current string) error {
	...
	for i, key := range sortKeys(object) {
		if i > 0 {
			buffer.WriteString(",\n")
		}
...
// buildYAMLNode builds a node tree so YAML follows the same order. yaml.v3 sorts map
...
func buildYAMLNode(value any) (*yaml.Node, error) {
	...
	case map[string]any:
		...
		for _, key := range sortKeys(typed) {
			child, err := buildYAMLNode(typed[key])
			if err != nil {
				return nil, err
			}
...
// sortsBefore ranks identifying fields first and timestamps last, with
...
func sortsBefore(a, b string) bool {
	ba, bb := band(a), band(b)
	if ba != bb {
		return ba < bb
	}
