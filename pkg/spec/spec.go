...
type Operation struct {
	...
	// inputs is the parameter split, computed once when the contract is read.
	inputs *Inputs
}
...
func newService(name string, doc *openapi3.T) *Service {
	...
	for _, p := range doc.Paths.InMatchingOrder() {
		...
		for method, op := range item.Operations() {
			...
			operation.inputs = newInputs(operation)
...
// readBaseURL reads the first entry of servers, which is where OpenAPI states
// an API's address. Empty when the contract does not say; callers report that
// rather than guessing.
func readBaseURL(doc *openapi3.T) string {
	if doc == nil {
		return ""
	}
...
// readVersion reads info.version, which is where a contract states its own
// version.
func readVersion(doc *openapi3.T) string {
