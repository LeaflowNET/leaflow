...
that is bounded by their lifetime and is not something a client can shorten.`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return ext.Printer.Print(map[string]any{"logged_out": true})
		},
	}
}
...
inspecting state should not change it.`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if !status.ProjectExpires.IsZero() {
				result["project_expires"] = status.ProjectExpires.Format(time.RFC3339)
			}
