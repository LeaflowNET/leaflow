...
func classify(err error) Failure {
	...
	case errors.Is(err, context.Canceled):
		return Failure{Kind: "interrupted", Message: "interrupted", ExitCode: ExitInterrupted}
	...
	case errors.Is(err, auth.ErrAccountTokenInCI):
		return Failure{Kind: "auth", Message: err.Error(), ExitCode: ExitAuth}
	...
	case errors.Is(err, config.ErrConfigNotFound),
		...
		return Failure{Kind: "usage", Message: err.Error(), ExitCode: ExitUsage}
	}

	return Failure{Kind: "error", Message: err.Error(), ExitCode: ExitError}
}

func fromAPI(err *transport.APIError) Failure {
	failure := Failure{
		Kind:     "api",
		Code:     err.Code,
		Status:   err.Status,
	...
	switch err.Code {
	case "TOKEN_MISSING", "TOKEN_EXPIRED", "TOKEN_INVALID", "IDENTITY_MISSING", "IDENTITY_INVALID":
		failure.Kind = "auth"
	...
	case "NOT_A_MEMBER":
		...
		return failure
	case "USER_SUSPENDED", "USER_BANNED":
		failure.Kind = "permission"
		failure.ExitCode = ExitPermission
...
func (a *App) report(err error, format output.Format) int {
	...
	case output.FormatJSON, output.FormatYAML:
		printer := &output.Printer{Format: format, Out: a.Err}
		if printErr := printer.Print(map[string]any{"error": failure}); printErr == nil {
