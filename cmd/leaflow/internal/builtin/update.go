...
replace a file it believes it owns.`,
	...
	cmd.RunE = func(cmd *cobra.Command, _ []string) error {
		latest, err := fetchLatestVersion(cmd.Context())
		if err != nil {
			return err
		}
...
func replaceSelf(ctx context.Context, version string) (string, error) {
	...
	if err := checkWritable(filepath.Dir(self)); err != nil {
		return "", err
	}

	// Whatever the last update displaced, on the platforms that leave something
	// behind. Done before the download so a leftover that cannot be removed is
	// reported in a second rather than after pulling twenty megabytes.
	if err := sweep(self); err != nil {
		return "", err
	}

	number := strings.TrimPrefix(version, "v")
	name := archiveName(number)
	...
	// Written beside the target so the move stays on one filesystem.
	staged := self + ".new"
	...
	if err := replaceBinary(self, staged); err != nil {
		if removed := os.Remove(staged); removed != nil {
...
// archiveName is what the release publishes for this platform.
//
// Windows gets a zip and everything else a tarball, because that is what
// goreleaser is configured to build — asking for the wrong one is a 404, which
// reads as "no such release" rather than "wrong file name".
func archiveName(number string) string {
	extension := "tar.gz"
	if runtime.GOOS == "windows" {
		extension = "zip"
	}

	return fmt.Sprintf("leaflow_%s_%s_%s.%s", number, runtime.GOOS, runtime.GOARCH, extension)
}

// pickBinaryName is what the archive calls the binary inside it.
func pickBinaryName() string {
	if runtime.GOOS == "windows" {
		return "leaflow.exe"
	}
...
// checkWritable reports whether a new file can be created in dir, which is what the
