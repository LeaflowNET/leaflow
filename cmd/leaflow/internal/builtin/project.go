...
worse than any error.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			id, name, err := findProject(cmd, ext, args[0])
			if err != nil {
				return err
			}
...
// findProject accepts an id or a name. Names are matched by listing, which
// costs one request but means people can use the word they actually know.
func findProject(cmd *cobra.Command, ext *extension.Context, wanted string) (id, name string, err error) {
	listed, err := listProjects(cmd, ext)
	if err != nil {
		return "", "", err
	}

	for _, item := range listProjectItems(listed) {
...
func listProjectItems(listed any) []any {
