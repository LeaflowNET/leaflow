...
func (a *App) Run(args []string) int {
	...
	if err := root.ExecuteContext(ctx); err != nil {
		return a.report(err, a.pickErrorFormat())
	}

	return ExitOK
}

// pickErrorFormat decides how a failure is rendered. It re-parses the flag instead
...
func (a *App) pickErrorFormat() output.Format {
	if a.outputName == "" {
...
// buildKernel assembles the client, the credentials and the printer, once the
// flags that decide them have been parsed.
//
...
func (a *App) buildKernel(cmd *cobra.Command) error {
	...
	client := transport.New(cfg.Context(), tokens, httpClient)
	...
	printer := &output.Printer{
		...
		Columns: columns,
		Out:     cmd.OutOrStdout(),
		Width:   measureTerminal(cmd.OutOrStdout()),
...
// measureTerminal returns 0 when stdout is not a terminal — a pipe has no width,
// and a table written into one should not be trimmed to fit a screen nobody is
// looking at.
func measureTerminal(out io.Writer) int {
	file, ok := out.(*os.File)
	if !ok {
		return 0
	}
